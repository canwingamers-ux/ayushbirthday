  ...
  return (
    <div className={`min-h-screen ${themeConfig.bg} ${themeConfig.text} overflow-hidden relative font-sans transition-colors duration-1000`}>
      ...
      <AnimatePresence mode="wait">
        ...
        {/* @ts-ignore */}
        {scene === 5 && <Scene4InteractiveCandle key="scene5" onComplete={handleCandleBlown} />}
        {/* @ts-ignore */}
        {scene === 6 && <Scene5PhotoCollage key="scene6" friend={friend} onNext={nextScene} />}
        {/* @ts-ignore */}
        {scene === 7 && <Scene6Closing key="scene7" friend={friend} onComplete={nextScene} />}
        {/* @ts-ignore */}
        {scene === 8 && <Scene7Goodbye key="scene8" friend={friend} />}
      </AnimatePresence>
    </div>
  );
}
